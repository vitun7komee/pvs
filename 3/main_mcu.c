...
{
    if (current_speed == 0) {
        _delay_ms(delay_mode[0]);
    } else {
        _delay_ms(delay_mode[current_speed - 1]);
    }
}
...
int main()
{
    ...
    while (1)
    {
        ...
        while (!(UCSRA & (1 << UDRE))) {} // Подождать, пока буфер передачи освободится
    }
